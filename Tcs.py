...
st.markdown('In last 90 days, the average closing price for Tata Consultancy Services stock was about ₹2915.For about 75% of time the stock was trading below ₹3139 and it clocked maximum of ₹3308.The maximum volume of shares traded on a single day was 9508039 with median quantity being 3113242.')
TCS_df = df.round(2)
# TCS_df.head(2)
...
TCS_df.dropna(inplace = True, axis = 0)

# TCS_df.dtypes

TCS_df.index = pd.to_datetime(TCS_df.index)
# TCS_df.head(2)

TCS_df.index.max() - TCS_df.index.min()

TCS_df.iloc[-90:].describe().astype(int)

TCS_df.index = TCS_df.index
...
st.markdown('Now we plot the closing price (adjusted) of the stock over the period of 2 year to get a general idea of how the stock performed in the given period')
# TCS_df["Adj Close"].plot(figsize = (8,5))
fig1 = plt.figure(figsize=(12,6))
plt.xlabel("TCS Stock- Adj Close")
plt.ylabel("Closing Prices")
plt.plot(TCS_df['Adj Close'])
plt.grid()
# plt.show()
st.pyplot(fig1)
...
TCS_df["Day_Perc_Change"] = TCS_df["Adj Close"].pct_change()*100
# TCS_df.head()


TCS_df.dropna(axis = 0, inplace = True)



fig2 = plt.figure(figsize = (12, 6))
plt.xlabel("TCS Stock-Daily Percent Change")
plt.ylabel("Daily Returns")
plt.plot(TCS_df["Day_Perc_Change"])
plt.grid()
st.pyplot(fig2)
...
# fig3 = TCS_df["Day_Perc_Change"].hist(bins = 50, figsize = (10,5)) 

fig3, ax = plt.subplots()
# ax.xlabel("Daily returns")
# ax.ylabel("Frequency")
ax.hist(TCS_df["Day_Perc_Change"], bins=30)
st.pyplot(fig3)
#satistics
TCS_df.Day_Perc_Change.describe()


st.markdown('The daily returns histogram is centered about origin. For the past 1 year, the mean daily returns has been about 0.185 and for most of the days the daily return was less than 2.5% implying that the Tata Consultancy Services stock has been less volatile over the period. During the period, the highest % change in positive direction was observed to be 9.84 % and was 9.41% in negative direction. Clearly, we didn’t had any instances of ‘bull run’ or ‘bear drop’!')
...
def trend(x):
    if x > -0.5 and x <= 0.5:
        ...
        return "Bear drop"
TCS_df["Trend"]= np.zeros(TCS_df["Day_Perc_Change"].count())
TCS_df["Trend"]= TCS_df["Day_Perc_Change"].apply(lambda x:trend(x))
# TCS_df.head()




Rel_pie_data = TCS_df.groupby("Trend")
pie_label = sorted([i for i in TCS_df.loc[:, "Trend"].unique()])
...
st.markdown('For the period under consideration from March 2020 -March 2021, theTata Consultancy Services stock was among the top gainers for about 5.6 % of the time, and among the top losers for 4.8 %. For about 21.0 % of the time period, the stock has performed positively on a given day. Likewise, for most period of time (about 28.2%) the stock showed a very slight change in the price. These observations are consistent with the daily return histogram we saw above.')
...
ax5.stem(TCS_df.index, TCS_df["Day_Perc_Change"])
# ax5.ylabel("Daily Returns and Volume")
(TCS_df["Volume"]/1000000).plot(figsize = (15, 11), 
                                 color = "green", 
                                 alpha = 0.5)
ax4.grid()
st.pyplot(fig5)
...
def allStock():
    ...
    TCS_df=data.DataReader('CIPLA.NS', 'yahoo', start, end)
...
st.markdown('If you observe carefully, the plots in the lower triangular area are the same as the plots in the upper triangular area with just axes interchanged. So, analyzing either set of plots would suffice. The diagonal represents the histograms, just like the one seen above for Cipla stock.')
...
with col1:
    sns.jointplot(data=pct_chg_df, x="TCS.NS", y= "RELIANCE.NS",kind="reg")
    a=stats.pearsonr(pct_chg_df["TCS.NS"], pct_chg_df["RELIANCE.NS"])
    print(a)
    figA = plt.show()
    st.header("A")
    st.pyplot(figA)
...
with col2:
    sns.jointplot(data=pct_chg_df, x="TCS.NS", y= "EICHERMOT.NS",kind="reg")
    b=stats.pearsonr(pct_chg_df["TCS.NS"], pct_chg_df["EICHERMOT.NS"])
    print(b)
    figB = plt.show()
    st.header("B")
    st.pyplot(figB)
...
with col3:
    sns.jointplot(data=pct_chg_df, x="TCS.NS", y= "HINDUNILVR.NS",kind="reg")
    c=stats.pearsonr(pct_chg_df["TCS.NS"], pct_chg_df["HINDUNILVR.NS"])
    print(c)
    figC = plt.show()
    st.header("C")
    st.pyplot(figC)
...
with col1:
    sns.jointplot(data=pct_chg_df, x="TCS.NS", y= "CIPLA.NS",kind="reg")
    d=stats.pearsonr(pct_chg_df["TCS.NS"], pct_chg_df["CIPLA.NS"])
    print(d)
    figD = plt.show()
    st.header("D")
    st.pyplot(figD)
...
with col2:
    sns.jointplot(data=pct_chg_df, x="TCS.NS", y= "AMBUJACEM.NS",kind="reg")
    e=stats.pearsonr(pct_chg_df["TCS.NS"], pct_chg_df["AMBUJACEM.NS"])
    print(e)
    figE = plt.show()
    st.header("E")
    st.pyplot(figE)
...
st.markdown('Next we’ll see the comparative volatility analysis of Tata Consultancy Services stock with Reliance, Cipla,Eicher Motors, Hindustan Unilever, Ambuja Cement stock and NIFTY50 index. Just like above, we compute 7-day rolling mean, and standard deviation.')

fig8 = plt.figure(figsize = (15,7))
volatility_with_reliance = pct_chg_df[['TCS.NS', 'RELIANCE.NS', '^NSEI']].rolling(7).std()*np.sqrt(7)
...
volatility_with_eicher = pct_chg_df[['TCS.NS', 'EICHERMOT.NS', '^NSEI']].rolling(7).std()*np.sqrt(7)
...
volatility_with_hul = pct_chg_df[['TCS.NS', 'HINDUNILVR.NS', '^NSEI']].rolling(7).std()*np.sqrt(7)
...
volatility_with_tcs = pct_chg_df[['TCS.NS', 'CIPLA.NS', '^NSEI']].rolling(7).std()*np.sqrt(7)
...
volatility_with_ambuja = pct_chg_df[['CIPLA.NS', 'AMBUJACEM.NS', '^NSEI']].rolling(7).std()*np.sqrt(7)
plt.plot(volatility_with_ambuja)
plt.xlabel("TCS")
plt.ylabel("Amubuja Cements Ltd.")
st.pyplot(fig12)

st.markdown('You can observe that Tata Consultancy Services stock has lower volatility with Reliance, Cipla and Eicher Motors and has higher volatility with Hindustan Unilever and Ambuja Cements while the Nifty index has least volatility.')
