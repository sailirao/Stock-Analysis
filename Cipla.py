...
Cipla_df.index = pd.to_datetime(Cipla_df.index)
# Cipla_df.head(2)

Cipla_df.index.max() - Cipla_df.index.min()

Cipla_df.iloc[-90:].describe().astype(int)

Cipla_df.index = Cipla_df.index
...
st.markdown('Now we plot the closing price (adjusted) of the stock over the period of 2 year to get a general idea of how the stock performed in the given period')
# Cipla_df["Adj Close"].plot(figsize = (8,5))
fig1 = plt.figure(figsize=(12,6))
plt.xlabel("Cipla Stock- Adj Close")
plt.ylabel("Closing Prices")
plt.plot(Cipla_df['Adj Close'])
plt.grid()
# plt.show()
st.pyplot(fig1)
...
plt.xlabel("Cipla Stock-Daily Percent Change")
plt.ylabel("Daily Returns")
plt.plot(Cipla_df["Day_Perc_Change"])
plt.grid()
st.pyplot(fig2)

st.markdown('It can be observed that for most of the days, the returns are between -2.5% to 5 % with few spikes in between crossing 12.5% mark on positive side and -7.0 on negative side.')

st.markdown('Plotting daily returns distribution histogram —')


# fig3 = Cipla_df["Day_Perc_Change"].hist(bins = 50, figsize = (10,5)) 

fig3, ax = plt.subplots()
# ax.xlabel("Daily returns")
# ax.ylabel("Frequency")
ax.hist(Cipla_df["Day_Perc_Change"], bins=30)
st.pyplot(fig3)
#satistics
Cipla_df.Day_Perc_Change.describe()


st.markdown('The daily returns histogram is centered about origin. For the past 1 year, the mean daily returns has been about 0.305 and for most of the days the daily return was less than 3% implying that the Cipla stock has been less volatile over the period. During the period, the highest % change in positive direction was observed to be 13.03% and was 6.85% in negative direction. Clearly, we didn’t had any instances of ‘bull run’ or ‘bear drop’!')
...
Rel_pie_data = Cipla_df.groupby("Trend")
...
st.markdown('For the period under consideration from March 2020 -March 2022, the Cipla stock was among the top gainers for about 6.7% of the time, and among the top losers for 3.5%. For about 17.1% of the time period, the stock has performed positively on a given day. Likewise, for most period of time (about 30%) the stock showed a very slight change in the price. These observations are consistent with the daily return histogram we saw above.')
...
ax5.stem(Cipla_df.index, Cipla_df["Day_Perc_Change"])
# ax5.ylabel("Daily Returns and Volume")
(Cipla_df["Volume"]/1000000).plot(figsize = (15, 11), 
                                 color = "green", 
                                 alpha = 0.5)
ax4.grid()
st.pyplot(fig5)
...
with col1:
    sns.jointplot(data=pct_chg_df, x="CIPLA.NS", y= "RELIANCE.NS",kind="reg")
    a=stats.pearsonr(pct_chg_df["CIPLA.NS"], pct_chg_df["RELIANCE.NS"])
    print(a)
    figA = plt.show()
    st.header("A")
    st.pyplot(figA)
...
with col2:
    sns.jointplot(data=pct_chg_df, x="CIPLA.NS", y= "EICHERMOT.NS",kind="reg")
    b=stats.pearsonr(pct_chg_df["CIPLA.NS"], pct_chg_df["EICHERMOT.NS"])
    print(b)
    figB = plt.show()
    st.header("B")
    st.pyplot(figB)
...
with col3:
    sns.jointplot(data=pct_chg_df, x="CIPLA.NS", y= "HINDUNILVR.NS",kind="reg")
    c=stats.pearsonr(pct_chg_df["CIPLA.NS"], pct_chg_df["HINDUNILVR.NS"])
    print(c)
    figC = plt.show()
    st.header("C")
    st.pyplot(figC)
...
with col1:
    sns.jointplot(data=pct_chg_df, x="CIPLA.NS", y= "TCS.NS",kind="reg")
    d=stats.pearsonr(pct_chg_df["CIPLA.NS"], pct_chg_df["TCS.NS"])
    print(d)
    figD = plt.show()
    st.header("D")
    st.pyplot(figD)
...
with col2:
    sns.jointplot(data=pct_chg_df, x="CIPLA.NS", y= "AMBUJACEM.NS",kind="reg")
    e=stats.pearsonr(pct_chg_df["CIPLA.NS"], pct_chg_df["AMBUJACEM.NS"])
    print(e)
    figE = plt.show()
    st.header("E")
    st.pyplot(figE)
...
st.markdown('Next we’ll see the comparative volatility analysis of Cipla stock with Reliance, Eicher Motors, Hindustan Unilever, TCS, Ambuja Cement stock and NIFTY50 index. Just like above, we compute 7-day rolling mean, and standard deviation.')

fig8 = plt.figure(figsize = (15,7))
volatility_with_reliance = pct_chg_df[['CIPLA.NS', 'RELIANCE.NS', '^NSEI']].rolling(7).std()*np.sqrt(7)
...
volatility_with_eicher = pct_chg_df[['CIPLA.NS', 'EICHERMOT.NS', '^NSEI']].rolling(7).std()*np.sqrt(7)
...
volatility_with_hul = pct_chg_df[['CIPLA.NS', 'HINDUNILVR.NS', '^NSEI']].rolling(7).std()*np.sqrt(7)
...
volatility_with_tcs = pct_chg_df[['CIPLA.NS', 'TCS.NS', '^NSEI']].rolling(7).std()*np.sqrt(7)
...
st.markdown('You can observe that Cipla stock has higher volatility as compared to Eicher Motors, Hindustan Unilever Ltd, Tata Consultancy Services and Amubuja Cement except for Reliance where it has low volatility while the Nifty index has least volatility.')
