...
Hindu_df = df.round(2)
# Hindu_df.head(2)
st.markdown('In last 90 days, the average closing price for Hindustan Unilever stock was about ₹2246.For about 75% of time the stock was trading below ₹2361 and it clocked maximum of ₹2450.The maximum volume of shares traded on a single day was 5687861 with median quantity being 2228646.')
Hindu_df.shape

Hindu_df.isnull().sum()

Hindu_df.dropna(inplace = True, axis = 0)

# Hindu_df.dtypes

Hindu_df.index = pd.to_datetime(Hindu_df.index)
# Hindu_df.head(2)

Hindu_df.index.max() - Hindu_df.index.min()

Hindu_df.iloc[-90:].describe().astype(int)

Hindu_df.index = Hindu_df.index
...
st.markdown('Now we plot the closing price (adjusted) of the stock over the period of 2 year to get a general idea of how the stock performed in the given period')
# Hindu_df["Adj Close"].plot(figsize = (8,5))
fig1 = plt.figure(figsize=(12,6))
plt.xlabel("Hindustan Unilever Ltd. Stock- Adj Close")
plt.ylabel("Closing Prices")
plt.plot(Hindu_df['Adj Close'])
plt.grid()
# plt.show()
st.pyplot(fig1)
...
Hindu_df["Day_Perc_Change"] = Hindu_df["Adj Close"].pct_change()*100
# Hindu_df.head()


Hindu_df.dropna(axis = 0, inplace = True)



fig2 = plt.figure(figsize = (12, 6))
plt.xlabel("Hindustan Unilever Ltd. Stock-Daily Percent Change")
plt.ylabel("Daily Returns")
plt.plot(Hindu_df["Day_Perc_Change"])
plt.grid()
st.pyplot(fig2)

st.markdown('It can be observed that for most of the days, the returns are between -2% to 2 % with few spikes in between crossing 12% mark on positive side and -7.0 on negative side.¶')

st.markdown('Plotting daily returns distribution histogram —')


# fig3 = Hindu_df["Day_Perc_Change"].hist(bins = 50, figsize = (10,5)) 

fig3, ax = plt.subplots()
# ax.xlabel("Daily returns")
# ax.ylabel("Frequency")
ax.hist(Hindu_df["Day_Perc_Change"], bins=30)
st.pyplot(fig3)
#satistics
Hindu_df.Day_Perc_Change.describe()


st.markdown('The daily returns histogram is centered about origin. For the past 1 year, the mean daily returns has been about 0.026 and for most of the days the daily return was less than 2.5% implying that the Hindustan Unilever stock has been less volatile over the period. During the period, the highest % change in positive direction was observed to be 13.49 % and was 8.87% in negative direction. Clearly, we didn’t had any instances of ‘bull run’ or ‘bear drop’!¶')
...
st.markdown('We wish to see how the stock was trending in past 2 year. This can be visualized as a pie chart, with each sector representing the percentage of days each trend occurred. We’ll plot a pie chart for the ‘Trend’ column to visualize the relative frequency of each trend category.')
...
def trend(x):
    if x > -0.5 and x <= 0.5:
        ...
        return "Bear drop"
Hindu_df["Trend"]= np.zeros(Hindu_df["Day_Perc_Change"].count())
Hindu_df["Trend"]= Hindu_df["Day_Perc_Change"].apply(lambda x:trend(x))
# Hindu_df.head()




Rel_pie_data = Hindu_df.groupby("Trend")
pie_label = sorted([i for i in Hindu_df.loc[:, "Trend"].unique()])
...
st.markdown('For the period under consideration from March 2020 -March 2021, the Hindustan Unilever stock was among the top gainers for about 2.4 % of the time, and among the top losers for 3.6 %. For about 21.0 % of the time period, the stock has performed positively on a given day. Likewise, for most period of time (about 25.8%) the stock showed a very slight change in the price. These observations are consistent with the daily return histogram we saw above.')
...
ax5.stem(Hindu_df.index, Hindu_df["Day_Perc_Change"])
# ax5.ylabel("Daily Returns and Volume")
(Hindu_df["Volume"]/1000000).plot(figsize = (15, 11), 
                                 color = "green", 
                                 alpha = 0.5)
ax4.grid()
st.pyplot(fig5)
...
with col1:
    sns.jointplot(data=pct_chg_df, x="HINDUNILVR.NS", y= "RELIANCE.NS",kind="reg")
    a=stats.pearsonr(pct_chg_df["HINDUNILVR.NS"], pct_chg_df["RELIANCE.NS"])
    print(a)
    figA = plt.show()
    st.header("A")
    st.pyplot(figA)
...
with col2:
    sns.jointplot(data=pct_chg_df, x="HINDUNILVR.NS", y= "EICHERMOT.NS",kind="reg")
    b=stats.pearsonr(pct_chg_df["HINDUNILVR.NS"], pct_chg_df["EICHERMOT.NS"])
    print(b)
    figB = plt.show()
    st.header("B")
    st.pyplot(figB)
...
with col3:
    sns.jointplot(data=pct_chg_df, x="HINDUNILVR.NS", y= "CIPLA.NS",kind="reg")
    c=stats.pearsonr(pct_chg_df["HINDUNILVR.NS"], pct_chg_df["CIPLA.NS"])
    print(c)
    figC = plt.show()
    st.header("C")
    st.pyplot(figC)
...
with col1:
    sns.jointplot(data=pct_chg_df, x="HINDUNILVR.NS", y= "TCS.NS",kind="reg")
    d=stats.pearsonr(pct_chg_df["HINDUNILVR.NS"], pct_chg_df["TCS.NS"])
    print(d)
    figD = plt.show()
    st.header("D")
    st.pyplot(figD)
...
with col2:
    sns.jointplot(data=pct_chg_df, x="HINDUNILVR.NS", y= "AMBUJACEM.NS",kind="reg")
    e=stats.pearsonr(pct_chg_df["HINDUNILVR.NS"], pct_chg_df["AMBUJACEM.NS"])
    print(e)
    figE = plt.show()
    st.header("E")
    st.pyplot(figE)
...
st.markdown('Next we’ll see the comparative volatility analysis of Hindustan Unilever stock with Reliance, Cipla, Eicher Motors, TCS, Ambuja Cement stock and NIFTY50 index. Just like above, we compute 7-day rolling mean, and standard deviation.')

fig8 = plt.figure(figsize = (15,7))
volatility_with_reliance = pct_chg_df[['HINDUNILVR.NS', 'RELIANCE.NS', '^NSEI']].rolling(7).std()*np.sqrt(7)
plt.plot(volatility_with_reliance)
plt.xlabel("Hindustan Unilever Ltd.")
plt.ylabel("Reliance")
st.pyplot(fig8)


fig9 = plt.figure(figsize = (15,7))
volatility_with_eicher = pct_chg_df[['HINDUNILVR.NS', 'EICHERMOT.NS', '^NSEI']].rolling(7).std()*np.sqrt(7)
...
volatility_with_hul = pct_chg_df[['HINDUNILVR.NS', 'CIPLA.NS', '^NSEI']].rolling(7).std()*np.sqrt(7)
...
volatility_with_tcs = pct_chg_df[['HINDUNILVR.NS', 'TCS.NS', '^NSEI']].rolling(7).std()*np.sqrt(7)
...
volatility_with_ambuja = pct_chg_df[['HINDUNILVR.NS', 'AMBUJACEM.NS', '^NSEI']].rolling(7).std()*np.sqrt(7)
plt.plot(volatility_with_ambuja)
plt.xlabel("Hindustan Unilever Ltd.")
plt.ylabel("Amubuja Cements Ltd.")
st.pyplot(fig12)

st.markdown('You can observe that Hindustan Unilever stock has lower volatility as compared to Reliance, Cipla, Eicher Motors, Tata Consultancy Services and Amubuja Cement while the Nifty index has least volatility.')
